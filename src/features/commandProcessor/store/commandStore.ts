 ...
 * @internal
 */
export function __setDependencies(deps: CommandProcessorDependencies): void {
    ...
    try {
      switch (command.type) {
        ...
        case CommandType.Describe:
          ...
          if (!deps.objectDetector.isReady()) await deps.objectDetector.initialize();
          
          const tensorResult = await deps.captureFrameTensor(
            YOLO_INPUT_SIZE,
            YOLO_INPUT_SIZE
          );
          if (!tensorResult.ok) {
        ...
        case CommandType.Banknote:
          ...
          if (!deps.banknoteClassifier.isReady()) await deps.banknoteClassifier.initialize();
          ...
          if (!tensorResultBanknote.ok) {
            ...
            set({state: 'error', errorMessage: tensorResultBanknote.userMessage || 'Ошибка камеры'});
            break;
          }
        ...
        case CommandType.Navigate:
          ...
          if (navigationResult.ok) {
            await deps.tts.speak(`Построен маршрут. Направление: ${mockDestination}. Начните движение.`);
